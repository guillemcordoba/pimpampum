...
- **Defense (dual target)**: at resolution the defender picks an **ally** (guard: attacks on that ally also resolve against the defender's defense; penetrating damage hits the **defender**, not the ally) **or an enemy** (block: "els enemics bloquejats fan totes les seves tirades d'atac contra el defensor" — the blocked enemy loses target choice; attacks that don't choose targets, i.e. full-coverage AoE, are unaffected). Either way the defender **always also defends themselves**, rolling their defense dice separately against each incoming attack. Engine: `Character.guards` + `Character.blockedBy`, round-scoped, resolved at resolution time in speed order; `TargetRequirement 'defense'` drives the dual prompt.
- **Focus**: special effects, usually slow; cancelled if the actor takes an undefended hit before it resolves (an impact interrupts even at 0 post-armour damage).
...
- **Fatigue** (`fatigue.ts`): a daily stamina budget, never a dice modifier. Every action played (in or out of combat) adds its `fatigueCost` (default 1; esgotadora cards declare 2-4, shown on the card). `FATIGUE_CONFIG.max = 15` is THE pacing knob (~2-3 combats/day); a card that would exceed it is unplayable. Fatigue persists across combats; `Character.sleep()` clears it. It NEVER touches rolls — roll penalties were measured to freeze fights into draws and are permanently rejected (intentions.md). **Exhaustion ending**: a round where nobody can play a card ends the combat — the team keeping the higher fraction of its total PV wins; an exact tie is a draw.
...
- **Equipment**: slot-based (Tors/Cap/Braços/Cames/Mà principal/Mà secundària); gives passive armour, speed penalty, roll bonuses, and/or **weapon dice** (actions with the `weapon_damage` effect attack with the wielded weapon's dice instead of their own — rendered as "arma" on the card).
...
- **Balancer v2 model** (`generator.ts`, all constants measured by the simulator's `measure-threat.ts` / `measure-model-accuracy.ts`): per-body threat normalized so a 4-player party's strength is 1; party strength S(n) = (n/4)^1.5; winrate = logistic with k = 2.65 on the threat/strength ratio; threat is SUPER-linear in bodies — a shared-economy β-power-mean over ALL bodies (β = 2.0), counts snap to the measured probe counts (horda 6 / elit 3 / solitari 1) with PV multiplier as the fine lever (clamped 0.4-3); `levelFactor ≈ 0.3 + 0.7·(level/kit)` is a rough global fit.
...
2. `planActions(humanSelections)` — build the pending queue (humans supply action ids; AI fills the rest via `selectAction`), **shuffle it, then sort by speed** (tie fairness); returns `RevealedAction[]`. Detects the exhaustion ending (empty queue, nobody stunned). Between plan and resolve, `flowSwapRefs()`/`flowSwap()` let card-swap statuses (Estat de flux) replace a revealed card.
