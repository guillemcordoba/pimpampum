...
- **Balancer v2 model** (`generator.ts`, all constants measured by the simulator's `measure-threat.ts` / `measure-model-accuracy.ts` / `measure-player-level.ts`; re-measured 2026-07-19 after the card-review pass): per-body threat normalized so a 4-reference-player party's strength is 1 (all threats RE-MEASURED 2026-07-19 after the armour rescale — parties are squishier now, so threats jumped: goblin .24, basilisk .69, horned-devil .80…); party strength S = armorFactor · (Σ g(levels_i)/4)^2.1 (α measured with FIXED-count probes — count-drift probes mix in β error) where the optional per-player input g(levels) = 0.85 + 0.15·(levels/6) (nearly flat: the first cards carry the player; ≡ 1 at the reference ~6 levels/player) and `armorFactor(avgArmor) = 1 + 0.28·(avgArmor − 0.75)` prices the party's average worn armour (a balancer INPUT; the calibration mean is +0.75, and the encounter creator equips the heroes to match on «Comença el combat»); winrate = logistic with k = 3.2 on the threat/strength ratio; threat is SUPER-linear in bodies — a shared-economy β-power-mean over ALL bodies (β = 2.0, known ~−8pp overpricing bias on mixed comps), counts snap HARD to the measured probe counts (horda 6 / elit 3 / solitari 1; max drift ×1.1 — per-kit count-scaling variance measured 20-30pp both ways when counts drift) with PV multiplier as the fine lever (clamped 0.4-3). **Enemy strength scales by LEVELS, not PV** (design decision 2026-07-19): kits RAMP — real power sits in the top cards (e.g. goblin 4-5: Punyalada traïdora `flanking`, Allau de la horda) — so level pricing is per-template MEASURED data: `EnemyTemplate.levelThreat` (sparse multipliers, `templateLevelFactor` interpolates; goblin L≤3 ×0.62) with the global `levelFactor = min(1, 0.4 + level/kit)` only as the flat-kit fallback. The calibration guards cover 4-player solves per template PLUS party sizes 3/5/6 and the per-player-levels path (the browser inputs).
