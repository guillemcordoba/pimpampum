...
export const ESGRIMA: SkillDefinition = {
  ...
  actions: [
    ...
    action({ id: 'atac-ampli', name: 'Atac ampli', skillId: 'esgrima', unlock: 20, type: ActionType.Atac, speed: -1, damage: d(1, 4), targetCount: 2, fatigueCost: 2, desc: "Ataca fins a 2 enemics amb un gran arc d'espasa.", icon: 'lorc/sword-spin.svg' }),
    action({ id: 'escomesa-furiosa', name: 'Escomesa furiosa', skillId: 'esgrima', unlock: 35, type: ActionType.Atac, speed: 2, damage: d(1, 4), fatigueCost: 2, effects: [{ type: 'second_attack', params: { dice: d(1, 4), target: 'random' } }], desc: 'Ataca i realitza immediatament un segon atac contra un enemic aleatori.', icon: 'lorc/spinning-sword.svg' }),
  ],
};
...
export const TACTICA_MILITAR: SkillDefinition = {
  ...
  actions: [
    ...
    action({ id: 'crit-de-guerra', name: 'Crit de guerra', skillId: 'tactica-militar', unlock: 25, type: ActionType.Focus, speed: -4, fatigueCost: 2, effects: [
...
export const COMBAT_SALVATGE: SkillDefinition = {
  ...
  actions: [
    action({ id: 'gran-escomesa', name: 'Gran escomesa', skillId: 'combat-salvatge', unlock: 1, type: ActionType.Atac, speed: -1, damage: d(1, 6), targetCount: 2, fatigueCost: 2, desc: "Colpeja fins a 2 enemics amb un gran arc de destral.", icon: 'lorc/battle-axe.svg' }),
    action({ id: 'atac-temerari', name: 'Atac Temerari', skillId: 'combat-salvatge', unlock: 20, type: ActionType.Atac, speed: -1, damage: d(1, 10), fatigueCost: 2, effects: [{ type: 'reckless', params: { attack: 0, defense: 3 } }], desc: '{D}-3 aquest torn i el següent.', icon: 'lorc/hammer-drop.svg' }),
...
export const FURIA: SkillDefinition = {
  ...
  actions: [
    action({ id: 'rabia', name: 'Ràbia', skillId: 'furia', unlock: 1, type: ActionType.Focus, speed: -5, fatigueCost: 2, effects: [
    ...
    action({ id: 'venjanca', name: 'Venjança', skillId: 'furia', unlock: 20, type: ActionType.Defensa, speed: 5, rollBonus: 1, fatigueCost: 2, effects: [
    ...
    action({ id: 'rugit-intimidant', name: 'Rugit intimidant', skillId: 'furia', unlock: 30, type: ActionType.Focus, speed: 1, fatigueCost: 2, effects: [
...
export const ARTS_MARCIALS: SkillDefinition = {
  ...
  actions: [
    ...
    action({ id: 'rafega-de-cops', name: 'Ràfega de cops', skillId: 'arts-marcials', unlock: 30, type: ActionType.Atac, speed: 1, damage: d(1, 4, -1), fatigueCost: 2, effects: [{ type: 'second_attack', params: { dice: d(1, 4, -1), target: 'same' } }], desc: 'Ataca el mateix enemic dues vegades.', icon: 'lorc/fulguro-punch.svg' }),
    action({ id: 'cop-de-silenci', name: 'Cop de silenci', skillId: 'arts-marcials', unlock: 40, type: ActionType.Atac, speed: 1, damage: d(1, 4), fatigueCost: 2, effects: [{ type: 'silence_on_hit', params: { turns: 2 } }], desc: 'Si fa mal, les cartes Focus del defensor es cancel·len automàticament els proper 2 torns.', icon: 'lorc/silence.svg' }),
  ],
};
...
export const KI: SkillDefinition = {
  ...
  actions: [
    ...
    action({ id: 'postura-del-vent', name: 'Postura del vent', skillId: 'ki', unlock: 25, type: ActionType.Focus, speed: 2, rollBonus: 5, fatigueCost: 2, effects: [
      { type: 'evasion', params: {} },
...
export const EARTHBENDER: SkillDefinition = {
  ...
  actions: [
    ...
    action({ id: 'empenta-de-terra', name: 'Empenta de terra', skillId: 'earthbender', unlock: 5, type: ActionType.Atac, speed: 1, damage: d(1, 4), effects: [{ type: 'debuff_on_hit', params: { kind: 'speed', amount: 2, duration: 'nextTurn' } }], desc: "Una onada de terra empeny l'enemic. Si fa mal, rep {V}-2 el pròxim torn.", icon: 'delapouite/boulder-dash.svg' }),
    action({ id: 'muralla-de-pedra', name: 'Muralla de pedra', skillId: 'earthbender', unlock: 8, type: ActionType.Defensa, speed: 1, rollBonus: 3, effects: [{ type: 'self_armor', params: { amount: 1 } }], desc: "Alces un mur de pedra davant d'un aliat. Guanyes 1 d'armadura aquest torn.", icon: 'delapouite/stone-wall.svg' }),
    action({ id: 'sentit-sismic', name: 'Sentit sísmic', skillId: 'earthbender', unlock: 12, type: ActionType.Focus, speed: 2, effects: [{ type: 'skill_mod', params: { kind: 'defense', amount: 2, target: 'self', duration: 3 } }], desc: 'Notes les vibracions del terra amb els peus descalços. {D}+2 durant 3 torns.', icon: 'lorc/barefoot.svg' }),
    action({ id: 'sorra-encegadora', name: 'Sorra encegadora', skillId: 'earthbender', unlock: 16, type: ActionType.Atac, speed: 2, damage: d(1, 4), effects: [{ type: 'debuff_on_hit', params: { kind: 'attack', amount: 3, duration: 'nextTurn' } }], desc: "Llances sorra als ulls de l'enemic. Si fa mal, rep {A}-3 el pròxim torn.", icon: 'delapouite/sandstorm.svg' }),
    action({ id: 'punxes-de-roca', name: 'Punxes de roca', skillId: 'earthbender', unlock: 20, type: ActionType.Atac, speed: -1, damage: d(1, 6), targetCount: 2, fatigueCost: 2, desc: 'Fas brollar punxes de pedra del terra sota 2 enemics que triïs.', icon: 'lorc/crags.svg' }),
    action({ id: 'pilar-de-roca', name: 'Pilar de roca', skillId: 'earthbender', unlock: 25, type: ActionType.Atac, speed: -1, damage: d(1, 6), fatigueCost: 2, effects: [{ type: 'stun_on_hit', params: { turns: 1 } }], desc: "Un pilar de pedra brolla i atrapa l'enemic. Si fa mal, queda atordit el pròxim torn.", icon: 'lorc/stone-tower.svg' }),
    action({ id: 'allau-de-roques', name: 'Allau de roques', skillId: 'earthbender', unlock: 30, type: ActionType.Atac, speed: -2, damage: d(1, 10), desc: "Fas caure una allau de roques sobre l'enemic. Lenta però devastadora.", icon: 'delapouite/falling-rocks.svg' }),
    action({ id: 'sorres-movedisses', name: 'Sorres movedisses', skillId: 'earthbender', unlock: 35, type: ActionType.Focus, speed: -2, fatigueCost: 2, effects: [
      { type: 'skill_mod', params: { kind: 'speed', amount: -4, target: 'enemy', duration: 2 } },
      { type: 'skill_mod', params: { kind: 'defense', amount: -3, target: 'enemy', duration: 2 } },
    ], desc: 'Tria un enemic. El terra se li enfonsa: {V}-4 i {D}-3 durant els pròxims 2 torns.', icon: 'lorc/swallow.svg' }),
    action({ id: 'armadura-de-roca', name: 'Armadura de roca', skillId: 'earthbender', unlock: 40, type: ActionType.Focus, speed: -3, fatigueCost: 2, effects: [{ type: 'skill_mod', params: { kind: 'armor', amount: 2, target: 'self', duration: 'restOfCombat' } }], desc: "Et cobreixes el cos amb plaques de pedra: +2 d'armadura per la resta del combat.", icon: 'delapouite/rock-golem.svg' }),
    action({ id: 'control-del-metall', name: 'Control del metall', skillId: 'earthbender', unlock: 45, type: ActionType.Atac, speed: -1, damage: d(1, 8), effects: [
      { type: 'piercing', params: {} },
      { type: 'bonus_damage', params: { amount: 2 } },
    ], desc: "Doblegues les impureses del metall de l'armadura enemiga. +2 de dany i ignora completament l'armadura passiva.", icon: 'delapouite/metal-golem-head.svg' }),
...
export const TIR_AMB_ARC: SkillDefinition = {
  ...
  actions: [
    ...
    action({ id: 'dagues-llancades', name: 'Dagues', skillId: 'tir-amb-arc', unlock: 15, type: ActionType.Atac, speed: 3, damage: d(1, 4), targetCount: 2, fatigueCost: 2, desc: 'Afecta a 2 enemics que triïs.', icon: 'lorc/daggers.svg' }),
  ],
};
...
export const ALQUIMIA: SkillDefinition = {
  ...
  actions: [
    ...
    action({ id: 'enverinar-arma', name: 'Enverinar arma', skillId: 'alquimia', unlock: 20, type: ActionType.Focus, speed: -4, fatigueCost: 2, effects: [{ type: 'weapon_buff', params: { amount: 1, count: 3, turns: -1 } }], desc: 'Tria 3 aliats (pot ser tu). Durant la resta del combat, els seus atacs físics fan perdre una vida addicional.', icon: 'lorc/poison-bottle.svg' }),
  ],
};
