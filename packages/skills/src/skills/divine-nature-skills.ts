...
export const FE_DIVINA: SkillDefinition = {
  ...
  actions: [
    action({ id: 'aura-protectora', name: 'Aura protectora', skillId: 'fe-divina', unlock: 1, type: ActionType.Defensa, speed: 1, rollBonus: 3, targetCount: 10, fatigueCost: 2, desc: 'Defensa tots els aliats a la vegada.', icon: 'lorc/beams-aura.svg' }),
    ...
    action({ id: 'imposicio-de-mans', name: 'Imposició de mans', skillId: 'fe-divina', unlock: 25, type: ActionType.Focus, speed: -2, fatigueCost: 2, effects: [
      { type: 'heal', params: { mode: 'roll', target: 'ally' } },
      { type: 'cleanse', params: { target: 'ally' } },
    ], desc: "Tria un aliat. Tira d20 + habilitat - fatiga. Cura el total ÷ 4 i elimina efectes negatius.", icon: 'lorc/glowing-hands.svg' }),
    action({ id: 'jurament-sagrat', name: 'Jurament sagrat', skillId: 'fe-divina', unlock: 40, type: ActionType.Focus, speed: -4, fatigueCost: 2, effects: [{ type: 'skill_mod', params: { kind: 'attack', amount: 2, target: 'self', duration: 'restOfCombat' } }], desc: 'Guanyes {A}+2 i {A}+2 per la resta del combat.', icon: 'lorc/aura.svg' }),
  ],
};
...
export const MAGIA_NATURA: SkillDefinition = {
  ...
  actions: [
    ...
    action({ id: 'lligams-natura', name: 'Lligams de natura', skillId: 'magia-natura', unlock: 35, type: ActionType.Focus, speed: -2, fatigueCost: 2, effects: [
...
export const FORMA_SALVATGE: SkillDefinition = {
  ...
  actions: [
    ...
    action({ id: 'forma-salvatge-canvi', name: 'Forma salvatge', skillId: 'forma-salvatge', unlock: 15, type: ActionType.Focus, speed: -4, fatigueCost: 3, effects: [{ type: 'wild_shape', params: { pv: 1, attack: 4, defense: 1 } }], desc: 'Transformació en bèstia: {A}+4, {D}+1 i +1 PV per la resta del combat. Un sol ús.', icon: 'lorc/werewolf.svg' }),
  ],
};
