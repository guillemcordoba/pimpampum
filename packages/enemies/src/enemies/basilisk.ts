  ...
  petrify_gaze: {
    ...
    onResolve(ctx) {
      ...
      for (const t of ctx.engine.enemiesOf(ctx.source)) {
        const gazeRaw = ctx.engine.rollContestDice(ctx.source, ctx.action.dice, 'save')
          + ctx.source.getRollBonus(ctx.action.skillId);
        const saveRaw = ctx.engine.rollContestDice(t, resist, 'save');
...
const BASILISK_SKILL: SkillDefinition = {
  ...
  actions: [
    action({ id: 'esclafament', name: 'Esclafament', skillId: 'basilisk', unlock: 1, type: ActionType.Atac, speed: -3, dice: d(3, 6), desc: '', icon: 'lorc/stoned-skull.svg' }),
    ...
    action({ id: 'mossegada-verinosa', name: 'Mossegada verinosa', skillId: 'basilisk', unlock: 2, type: ActionType.Atac, speed: 0, dice: d(2, 8), effects: [{ type: 'poison_on_hit', params: { damage: 4, turns: 1, name: 'verí' } }], desc: "Si impacta, l'enemic perd 4 de vida addicional el següent torn.", icon: 'lorc/snake-bite.svg' }),
    action({ id: 'cop-de-cua', name: 'Cop de cua', skillId: 'basilisk', unlock: 3, type: ActionType.Atac, speed: 2, dice: d(2, 6), targetCount: 3, fatigueCost: 2, desc: 'Colpeja fins a 3 enemics amb la cua.', icon: 'lorc/spiked-tail.svg' }),
...
export const BASILISK: EnemyModule = {
  template: { id: 'basilisk', displayName: 'Basilisc', classCss: 'basilisc', iconPath: ICON + 'delapouite/spiked-dragon-head.svg', role: 'solitari', threat: 0.525, skills: ['basilisk'], basePV: 43, suggestedLevel: 5, naturalArmor: 2, aiStrategy: AIStrategy.Power },
