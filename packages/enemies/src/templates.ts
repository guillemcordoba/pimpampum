import { Character, ActionDefinition, createCharacter } from '@pimpampum/engine';
 ...
 * skills; any unset skill uses the template's suggestedLevel.
 */
export function createEnemyFromTemplate(template: EnemyTemplate, levels: Record<string, number> = {}, name = template.displayName): Character {
...
export function createEnemy(id: string, levels: Record<string, number> = {}, name?: string): Character | undefined {
  const t = templateIndex.get(id);
  return t ? createEnemyFromTemplate(t, levels, name) : undefined;
}
